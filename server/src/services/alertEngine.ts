...
export function checkSpendingAlerts(
  ...
  // Capitalize category to match budget keys
  const categoryKey = newCategory.charAt(0).toUpperCase() + newCategory.slice(1);
  const categoryBudget = budgets[categoryKey] ?? 5000;
  const categoryTotal =
    monthlyTxs
