...
export async function POST(request: NextRequest) {
  try {
    ...
    // Get user ID from Authorization header (optional for now, for backward compatibility)
    ...
    const stmt = db.prepare(`
      INSERT INTO transactions (id, user_id, raw_text, amount, category, merchant, date)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `);

    stmt.run(id, userId || null, raw_text, classified.amount, classified.category, classified.merchant, date);
